...
def engineer_features(df):
    ...
    string_to_date = lambda date_string: dt.datetime.strptime(date_string,'%b-%y')
    diff_month = lambda d1, d2: 12*(d1.year - d2.year) + d1.month - d2.month
    
    #Convert string data type to numerical where needed in calculations
    df['last_pymnt_d'] = df['last_pymnt_d'].apply(string_to_date)
    ...
    le = LabelEncoder()
    df['sub_grade_num'] = le.fit_transform(df['sub_grade'].as_matrix())
